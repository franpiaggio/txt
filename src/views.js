...
function botonGuardar(ctx, thread, guardado) {
  ...
  return html` · <form method="post" action="/h/${thread.id}/guardar" class="en-linea"><input type="hidden" name="_csrf" value="${ctx.csrf}">${guardado
    ? html`<input type="hidden" name="quitar" value="1"><button class="enlace">Sacar de guardados</button>`
    : html`<button class="enlace">Guardar</button>`}</form>`;
}
...
export function hilo(ctx, { thread, board, posts, ids, form, guardado = false }) {
  ...
  return html`<p class="ayuda"><a href="/b/${board.slug}">← ${board.nombre}</a>${botonGuardar(ctx, thread, guardado)}</p>
